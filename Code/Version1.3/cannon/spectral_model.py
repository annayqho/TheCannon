...
def draw_spectra(model, test_set):
    """Uses the fitted model to "draw" a best-fit spectrum for each test object.

    Parameters
    ----------
    model: 
    test_set: Dataset object
...
def diagnostics(cannon_set, test_set, model):
    """Runs diagnostics on the best-fit spectra."""
...
def triangle_pixels(cannon_set):
    ...
    fig = triangle.corner(fluxes)

def overlay_spectra(cannon_set, test_set, model):
    """ Overplot original test spec with best-fit spec.
...
def residuals(cannon_set, test_set):
    ...
    for i in range(len(cannon_set.label_names)):
        ...
        axScatter.set_title("Spectral Residuals Sorted by ${0:s}$".format(label_name))
