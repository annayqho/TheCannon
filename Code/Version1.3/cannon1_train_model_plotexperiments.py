...
def model_diagnostics(reference_set, model):
    ...
    ax.set_xlim(np.ma.min(lams), np.ma.max(lams))
    ...
    line2, = ax2.plot(lams, baseline_spec, c='k', linewidth=0.3,
                        label=r'$\theta_0$' + "= the leading fit coefficient")
    #dots2, = ax2.scatter(contpix_lambda, y, s=1, color='r',label="continuum pixels")

    def onselect(xmin, xmax):
        indmin, indmax = np.searchsorted(lams, (xmin, xmax))
        indmax = min(len(lams)-1, indmax)
        spec_x = lams.compressed()[indmin:indmax]
        spec_y = baseline_spec.compressed()[indmin:indmax]
        line2.set_data(spec_x, spec_y)
        #dots_x = contpix_lambda[indmin:indmax]
        #dots_y = y[indmin:indmax]
        #dots2.set_data(dots_x, dots_y)
        ax2.set_xlim(spec_x[0], spec_x[-1])
    ...
    plt.xlim(xmins[0], xmaxs[-1])
                #np.ma.min(lams), np.ma.max(lams))
    ax = axarr[0]
    ax.plot(lams, baseline_spec, c='k', linewidth=0.3,
             label=r'$\theta_0$' + "= the leading fit coefficient")
    ax.scatter(contpix_lambda, y, s=1, color='r',label="continuum pixels")
    ...
    filename = "baseline_spec_with_cont_pix_%s.png" %i
    ...
    for i in range(nlabels):
        label = label_names[i]
        texlabel = r'$\theta_%s$'%(i+1) + \
                '=coeff for ' + r" $%s$*%s"%(label_names[i], ratios[i])
        texlabels.append(texlabel)
        coeffs = coeffs_all[:,i+1] * ratios[i]
        first_order[:,i] = coeffs
        ax.plot(lams, coeffs, linewidth=0.3, label=texlabel)
    box = ax.get_position()
