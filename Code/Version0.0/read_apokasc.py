"""
This is...training on ASPCAP, testing on APOKASC. 

I need to change these files so that they're separate. There should be a file that simply lets you generate a spectra object from a list of data files. It shouldn't be "get_training_set" and "get_test_set" separately at all. 

This is the APOKASC implementation of the ReadData class, which establishes 
the training set and test set for input into The Cannon.

ReadAPOKASC inherits from ReadData. The user (me, in this case) has tailored 
...
class ReadAPOKASC(ReadData):
    ...
    def get_spectra(self, files):
        """
        Extracts spectra (wavelengths, fluxes, fluxerrs) from APOKASC fits files
