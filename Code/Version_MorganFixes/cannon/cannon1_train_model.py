...
def do_one_regression_at_fixed_scatter(lams, fluxes, ivars, lvec, scatter):
    ...
    fluxes: ndarray, [nstars]
    ivars: ndarray, [nstars]
...
def do_one_regression(lams, fluxes, ivars, lvec):
    ...
    fluxes: ndarray, [nstars]
    ivars: ndarray, [nstars]
    lvec = the label vector, ndarray, [nstars, 10]
...
def train_model(reference_set):
    ...
    Parameters
    ----------
    reference_set: a Dataset object (see dataset.py)
    ...
    # one per pix
    blob = map(do_one_regression, lams, fluxes, ivars, lvec_full) 
