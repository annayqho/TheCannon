...
test = ax.imshow(image.T, cmap=plt.cm.bwr_r, interpolation="nearest", vmin = -0.001, vmax = 0.001 ,aspect = 'auto',origin = 'lower')
cb = fig.colorbar(test) 
#cb.set_label("arcsinh($\chi$)", fontsize = 20 ) 
cb.set_label("data-mean", fontsize = 20 )
#fig.show()
fig.savefig('chisq_map.eps', transparent=True, bbox_inches='tight', pad_inches=0)
