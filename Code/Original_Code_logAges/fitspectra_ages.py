...
def get_goodness_fit(fn_pickle, filein, Params_all, n):
    fd = open(fn_pickle,'r')
...
def do_one_regression_at_fixed_scatter(data, features, scatter):
    ...
    chi = np.sqrt(Cinv) * (x - np.dot(M, coeff)) # that's the chi for each wavelength for every star - this is what I want for that grid plot 
...
if __name__ == "__main__":
    ...
    nstars = (testdataall.shape)[1]
    nlabels = len(labels)
    xdata = testdataall[:,00,0]
    n = len(xdata)
    print "len(xdata): %s" %str(n)
    print "Number of stars: %s" %str(nstars)
    print "Number of labels: %s" %str(nlabels)
    Params_all = np.zeros((nstars, nlabels))
    red_chisq = get_goodness_fit('coeffs_2nd_order.pickle', 'normed_data', Params_all, n) 
