...
scatter(x[0:15], sum(new_contmasks[0:15], axis=1))
pix = np.linspace(0, npix-1, npix)

for i in range(0, ntrials):
    scatter(pix, new_contmasks[i]-0.02*i)

for i in range(0, ntrials):
    plot(sample_spec[i]-i)


# the norm_tr_fluxes look like kind of a mess.
# plot the cont pixels in a grid and see what happened...
