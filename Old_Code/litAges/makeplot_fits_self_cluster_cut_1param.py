...
#    file_in = "self_tags.pickle"
    ...
    params_temp, covs_params_temp = pickle.load(file_in2)
    sp = shape(params_temp) 
    params_temp = array(params_temp)
    file_in2.close()
    ...
    # now we have cut away the elements with sketchy temperature values
    age = array(age)
    ...
    #the index where the cluster begins
    
    # If you want to pick a specific cluster...
    # Pleiades: 394 to 437
    start = 394
    end = 437

    params = array(params_temp)[pick]
    covs_params = array(covs_params_temp)[pick]
    sp2 = shape(params) 
    sp3 = len(t) 
    #covs_params = np.linalg.inv(icovs_params) 
    rcParams['figure.figsize'] = 12.0, 10.0
    fig, ax = pyplot.subplots()
    ax.set_title("Age Results from the Cannon")
    ...
    pickcluster = 'Pleiades'
    indc1 = np.where(names=='Pleiades')[0][0]
    indc2 = np.where(names=='Pleiades')[0][-1]
    pick = logical_and(g[indc1:indc2] > 0, logical_and(t_err[indc1:indc2] < 300, feh[indc1:indc2] > -4.0))
    
    x = input_ASPCAP[3][indc1:indc2][pick]
    y = params_labels[3][indc1:indc2][pick]

    print x
    print y

    ax.plot(input_ASPCAP[3][indc1:indc2][pick], params_labels[3][indc1:indc2][pick], 'ko')
...
#        ax.plot(input_ASPCAP[3][indc1:indc2][pick], params_labels[3][indc1:indc2][pick], plot_markers[i]) 
    ...
    ax.set_xlim(0.5, 15) 
    ax.set_ylim(0, 15) 
    ax.set_ylabel("Age [Gyr]", fontsize = 14,labelpad = 5) 
    ax.set_xlabel("Literature Ages [Gyr]", fontsize = 14,labelpad = 10) 
